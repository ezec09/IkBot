chrome.runtime.onMessage.addListener(mensajeRecibido);

function mensajeRecibido(request,sender,sendResponse){
    if(request.msg === "estasOn?")
        sendResponse({msg: 'Yeah'});
}
...
function inicializar() {
    ...
    obtener('hacerPiratas').then((resultado)=>{fabricarPiratas = resultado.hacerPiratas});
}
...
function ponerSolucionCaptcha() {
    ...
    getBotonAbordarCaptcha().onclick = function() {
                                esperarCondicion(function(){return(existeID("missionProgressBar") || otroCaptcha(urlAnterior))},tiempoCorto,expirar,10)
                                .then(finalizar)
                            };
